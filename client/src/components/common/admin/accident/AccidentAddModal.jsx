...
export default function AccidentAddModal({ isOpen, onClose }) {
  ...
  const [selectedVehicles, setSelectedVehicles] = useState([null]); // Array for multiple selections
  const {
    register,
    handleSubmit,
    reset,
  ...
  const handleVehicleChange = (index, event) => {
    ...
    updatedVehicles[index] = vehicleId;
    setSelectedVehicles(updatedVehicles);

    // Add another dropdown if a vehicle was selected and there are no unselected dropdowns
    if (index === selectedVehicles.length - 1 && vehicleId) {
      setSelectedVehicles([...updatedVehicles, null]);
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Create accident-vehicle relations for all selected vehicles
      await Promise.all(
        selectedVehicles
            ...
            {selectedVehicles.map((selectedVehicle, index) => (
              ...
              >
                <option value="" disabled>
                  {index === 0 ? "Select Vehicle" : "Select Additional Vehicle"}
                </option>
